  ...
  <OverlayPanel ref="overlayPanel">
    <div class="flex flex-column gap-2 overlay">
      ...
      <div class="flex gap-2">
        <InlineMessage v-if="error"  class="flex-grow-1 flex-shrink-1" severity="error">{{t('journal.createEntryError')}}</InlineMessage>
        <span v-else class="flex-grow-1"></span>
        <Button :label="t('journal.createEntry')" @click="createEntry" :disabled="!selectedTrack"></Button></div>
    </div>
  </OverlayPanel>
</template>
