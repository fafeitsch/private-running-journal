...
const formattedEntries = computed(() => entries.value.map(entry => ({...entry, length: (entry.length / 1000).toFixed(1)})))
</script>
...
<template>
  ...
  <ul v-else class="list-none p-0 mt-3 h-full overflow-auto">
    <li v-for="entry of formattedEntries" :key="entry.id" class="list-none p-0 m-0 flex flex-column">
      <RouterLink
        v-ripple
        ...
        <span class="font-normal flex-shrink-1 overflow-hidden white-space-nowrap text-overflow-ellipsis">{{ entry.trackName }}</span
        ><span class="font-light flex-grow-1 flex-shrink-1 text-overflow-ellipsis overflow-hidden white-space-nowrap">&nbsp;–&nbsp;{{ entry.trackVariant }}</span>
