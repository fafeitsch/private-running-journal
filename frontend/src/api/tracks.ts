...
export function useTracksApi() {
  ...
  async function getTrack(id: string): Promise<tracks.Track>  {
   return GetTrack(id);
  }
  ...
  return { getTrackTree,getTrack, getGpxData, ComputePolylineProps, saveTrack, createTrack, deleteTrack, moveTrack };
}
